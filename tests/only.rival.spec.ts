...
test('--EVOLVE SKIN---', async({request})=> {
    ...
    let inventoryResponse = await MaketPlace.GET<Response>(`https://data.staging.thetanarena.com/theta/v1/inventory`,request, {}, tokenUser)

    

    // 2. get Minion
    let MinionResponse = await MaketPlace.GET<Response>(`https://data.staging.thetanarena.com/theta/v1/minion`, request, {}, tokenUser)
    ...
    let postedEvolve  = await Rival.PostEvolve<Response>(request, bodyPostedEvolve, tokenUser)
})