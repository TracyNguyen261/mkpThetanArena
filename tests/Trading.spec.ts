...
import { MathUtil, TextUtil, WaitUtil } from "../src/util/Util";
...
test("login", async ({ page, browser }) => {
  ...
  await page
    .locator(loginMetamaskBtn, { hasText: "Login with Metamask" })
    .click();
...
test("buy common box ", async ({ page, browser }) => {
  ...
  // await page.locator(TextUtil.ClosePopup().click())
  ...
  /* await page.goto("https://staging.marketplace.thetanarena.com/profile?tab=inventory&category=thetabox")
    // console.log("số lượng box:", TextUtil.GetIntFromText(await page.locator('span.cF3D5iGpvsF7xF_0g5fV').innerText()))
     await delay(3000)
    */
  ...
  await WaitUtil.ActAndWaitNewTab(
    page.locator(`//button[contains(.,"Checkout")]`).click(),
    metamask.browserContext
  );
  ...
  // kiểm tra số lượng box trong inventory
  await page.goto(
    "https://staging.marketplace.thetanarena.com/profile?tab=inventory&category=thetabox"
  );
  console.log(
    "số lượng box sau khi mua:",
    TextUtil.GetIntFromText(
      await page.locator("span.cF3D5iGpvsF7xF_0g5fV").innerText()
    )
  );
  await delay(10000);
});
...
test("----BUY HERO----", async ({ page, browser }) => {
  ...
  var pageTotal = TextUtil.GetIntFromText(
    await page.locator(pageTotalLct).innerText()
  );
  for (let i = 1; i < pageTotal; i++) {
    ...
    for (let j = 1; j < heroItem; j++) {
      ...
      if (priceHero < 50000) {
        ...
        await page.locator(heroItemMaketLct).nth(j).click();
        await delay(1000);
        var isOwnerCheck = await page
          .locator(`//span`, { hasText: "Owner by me" })
          .isVisible();
  ...
  await expect(
    page.locator(`//span`, { hasText: "Owner by me" })
  ).toBeVisible();
  ...
  // console.log("số lượng box sau khi mua:", TextUtil.GetIntFromText(await page.locator('span.cF3D5iGpvsF7xF_0g5fV').innerText()))
  // await delay(10000)
});
...
test("sell hero ", async ({ page, browser }) => {
  ...
  await delay(2000);
  await page.goto(
    "https://staging.marketplace.thetanarena.com/profile?tab=inventory"
  );
  ...
  await delay(20000);
  //await WaitUtil.ActAndWaitNewTab(page.locator(`//button[contains(.,"Checkout")]`).click(), metamask.browserContext)
  await metamask.__primaryMetamask();
  await delay(2000);
  ...
  // await page.locator(`//span[contains(.,"Close")]`).click()
  // await delay(2000)
  //  await WaitUtil.ActAndWaitNewTab(page.locator(`//span[contains(.,"I understand”)]`).click(), )
...
test("SELL HERO TRINH ", async ({ page, browser }) => {
  ...
  for (let i = 1; i <= pageTotal; i++) {
    ...
    for (let j = 0; j < heroCount; j++) {
      ...
      console.log(
        `let statusLabel = await page.locator('.Dfj5fSymG7FC_pQ1ltOn').nth(i).innerText()`
      );
...
test("---- RENT OUT HERO----- ", async ({ page, browser }) => {
  ...
  const rentingPrice = MathUtil.RandNumber(50, 100);
  console.log("- Price:", rentingPrice);
  await page
    .locator(`input.tyMBbWlgZp5BaqSW29K2`)
    .fill(rentingPrice.toString()); // input renting price
  await delay(2000);
  ...
  //Check price
  const priceRentOutHero = await page
    .locator(`span.f9zZck_3CSpfmtllo7B2`)
    .innerText();
  ...
  // Check label
  var labelRentOut = await page
    .locator(`//span`, { hasText: "Cancel renting" })
    .isVisible();
  if (labelRentOut) {
    await delay(1000);
  ...
  await delay(2000);
  await page.goto(
    "https://staging.marketplace.thetanarena.com/profile?tab=history"
  );
  await delay(2000);
  ...
  const heroIdInventory = await page
    .locator(`span.IpznSo8M9_7uqaFGDreR`)
    .first()
    .innerText();
...
test("--------CHECK HERO DETAIL------", async ({ page, browser }) => {
  ...
  await delay(2000);
  await page.goto(
    "https://staging.marketplace.thetanarena.com/item/6332de7d7af5e5ca7bb966b3"
  );
  await delay(2000);
  const heroDetail = page.url();
  await delay(2000);
